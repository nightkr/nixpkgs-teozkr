...
let
  ...
  # Modern freetype2 breaks old versions of Wine: https://bbs.archlinux.org/viewtopic.php?id=230675
  fixFreetype = ps: (ps.freetype.override {
    useEncumberedCode = false;
  }).overrideAttrs (old: rec {
    name = "freetype-2.7";
    src = fetchurl {
      url = "mirror://savannah/freetype/${name}.tar.bz2";
      sha256 = "0j3xgzn6pchgg1nm294vhx7cdicb7x3x8kwnlcm7v1alnzsm396n";
    };
  });

  deps = ps: (with ps; [ dbus zlib alsaLib fontconfig (fixFreetype ps) libpng12 libjpeg ]) ++ (with ps.xorg; [ libX11 libXext libXdamage libXrandr libXrender libXfixes libSM libXtst libXinerama libXau]);
...
stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    ...
    url = "https://dl.tvcdn.de/download/version_9x/teamviewer_linux.deb";
    sha256 = "0la4s8hzr030vgwsppwbkfp2s6y0v9dfbwhznkhs48qmgsgsdnb6";
  };
  ...
  installPhase = ''
    ...
    cp -a opt/teamviewer9/* $out/share/teamviewer
    rm -R \
      $out/share/teamviewer/logfiles \
      $out/share/teamviewer/config \
      $out/share/teamviewer/tv_bin/xdg-utils \
      $out/share/teamviewer/tv_bin/script/{teamviewer_setup,teamviewerd.sysv,teamviewerd.service,teamviewerd.*.conf,libdepend}
    ...
    sed -i "s,/opt/teamviewer9,$out/share/teamviewer,g" desktop/teamviewer-*.desktop
